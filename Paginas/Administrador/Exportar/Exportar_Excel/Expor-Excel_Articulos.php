<?php
	...
	$consulta = "SELECT articulos.idArticulo, tipoarticulos.Descripcion, marcas.Nombre, articulos.Nombre, `UnidadMedida`FROM `articulos` INNER JOIN tipoarticulos ON articulos.idTipoArticulo=tipoarticulos.idTipoArticulo INNER JOIN marcas ON articulos.idMarca=marcas.idMarca"; //Conexion a la base de datos 
	...
	if($resultado->num_rows > 0 ){
		...
		$objPHPExcel->getProperties()->setCreator("Siavicol") //Autor
							 ...
							 ->setTitle("Articulos Registrados")
							 ->setSubject("Articulos Registrados")
							 ->setDescription("Articulos de la unidad de avicultura")
							 ->setKeywords("Articulos")
		...
		$tituloReporte = "Articulos Registrados"; //titulo del Reporte 
		//titulos de cada columna 
		$titulosColumnas = array('Tipo De Insumo', 'Marca', 'Nombre', 'Unidad De Medida', 'Cantidad');
...
if ($fila1==true) {
	...
	$cantidad=0;
      ...
      //   			'wrap'          => true 
		...
		$objPHPExcel->getActiveSheet()->setTitle('Articulos Registrados');
		...
		header('Content-Disposition: attachment;filename="Articulos_Registrados-SIAVICOL.xlsx"');
