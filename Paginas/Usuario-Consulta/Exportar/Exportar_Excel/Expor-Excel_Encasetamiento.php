<?php
	...
	$consulta = "SELECT encasetamiento.idEncasetamiento,  galpon.NombreGalpon, manuales.NombreManual, `FechaInicio`, `FechaFin`, `Cantidad`, `NumeroSemanaVida`,DATEDIFF(DATE_FORMAT(NOW(),'%Y-%m-%d'),FechaInicio) as fecha_actual FROM `encasetamiento` inner join manuales on encasetamiento.IdManual=manuales.IdManual inner join galpon ON galpon.idGalpon=encasetamiento.idGalpon";
	...
	if($resultado->num_rows > 0 ){
		...
		$objPHPExcel->getProperties()->setCreator("Siavicol") //Autor
							 ...
							 ->setTitle("Encasetamientos Registrados")
							 ->setSubject("Encasetamientos Registrados")
							 ->setDescription("Encasetamientos de la unidad de avicultura")
							 ->setKeywords("Encasetamientos")
		...
		$tituloReporte = "Encasetamientos Registrados";
		$titulosColumnas = array('Galpon', 'Raza De La Gallina', 'Fecha De Inicio', 'Fecha Fin', 'Cantidad Inicial De animales', 'Cantidad Actual De animales', 'Edad De encasetamiento', 'Semanas De vida');
...
if ($fila1>0) {
	...
	$fila[7]=round(($fila[7]/7)+$fila[6]);
			$objPHPExcel->setActiveSheetIndex(0)
            		...
            		->setCellValue('F'.$i,  $cantidad_actual)
      ...
      //   			'wrap'          => true 
		...
		$objPHPExcel->getActiveSheet()->setTitle('Encasetamientos Registrados');
		...
		header('Content-Disposition: attachment;filename="Encasetamientos_Registrados-SIAVICOL.xlsx"');
