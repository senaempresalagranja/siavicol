...
$id_encasetamiento=$_POST["id_encasetamiento"];
 ...
 $connection=mysqli_connect("localhost","root","","siavicol");
    $query3="SELECT week(Fecha) as numero_semana FROM `produccion` where idEncasetamiento=$id_encasetamiento  GROUP BY numero_semana";
  ...
  $('#container').highcharts({
    ...
    xAxis: {
      categories: [
            ...
            $sql = "SELECT Fecha, dayname(Fecha) as dias  FROM `produccion` WHERE week(Fecha)=$semanas
    ...
    tooltip: {
      headerFormat: '<span style="font-size:10px">{point.key}</span><table>',
        ...
        series: [{
          ...
          $connection=mysqli_connect("localhost","root","","siavicol");
          $sql = "SELECT week(Fecha) as semana, SUM(CantidadTipoA) as huevos_A, SUM(`CantidadTipoB`) AS huevos_B, SUM(`CantidadTipoAA`) AS huevo_AA, SUM(`CantidadTipoAAA`) AS huevos_AAA, SUM(`CantidadTipoC`) AS huevo_C, SUM(`CantidadTipoJumbo`) AS huevos_Jumbo ,Fecha, dayname(Fecha) as dias FROM `produccion` WHERE week(Fecha)=$semanas AND idEncasetamiento=$id_encasetamiento GROUP BY dias ORDER BY Fecha";
          $result = mysqli_query($connection,$sql);
...
$cantidad_total=$cantidad_total/$canitdad_aves;
 ?>
          ...
          name: 'Produccion Experada',
          data: [      <?php

          $connection=mysqli_connect("localhost","root","","siavicol");
          $sql = "SELECT * FROM `parametros` where semana=$numero_semana AND  IdManual=$manuales order by semana";
          $result = mysqli_query($connection,$sql);
